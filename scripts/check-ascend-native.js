...
function worker(tabs,options={}){
  ...
  const runtime={id,getURL:file=>origin+file,lastError:null,onMessage:{addListener(fn){listener=fn;}},
    ...
    ...(options.platform?{getPlatformInfo:async()=>({os:options.platform})}:{})
  };
...
async function main(){
  ...
  const linux=worker(undefined,{storage:{},platform:'linux'});
  await until(()=>linux.state()==='HOST_NOT_REGISTERED');
  assert.equal(linux.ports.length,0);
  const linuxStatus=await linux.send({action:'STATUS'});
  assert.equal(linuxStatus.error_code,'NATIVE_HOST_WINDOWS_ONLY');
  assert.equal(linuxStatus.native_host_name,'com.freightdesk.ascend_x1');
  assert.equal(linuxStatus.production_connection_enabled,false);
  await linux.send({action:'CHECK_HOST'});
  assert.equal(linux.ports.length,0);
  assert.equal(linux.state(),'HOST_NOT_REGISTERED');
  const windows=worker(undefined,{storage:{},platform:'win'});
  await until(()=>windows.state()==='NOT_ENROLLED');
  assert.equal(windows.ports.length,0);
