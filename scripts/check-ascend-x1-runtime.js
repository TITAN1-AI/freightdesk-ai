...
const build={extension_version:'0.6.2',controller_revision:3,native_protocol:1,content_protocol:3};
...
function tabsFixture(items){
  ...
  const tabs={onRemoved:event(),onUpdated:event(),async query(){return items.filter(t=>t.url.startsWith('https://ascendtms.com/'));},
    async reload(number){reloads.push(number);const t=items.find(t=>t.id===number);if(t.blockRefresh)throw Error('PRIVATE_BROWSER_ERROR');t.version='0.6.2';t.missing=false;t.status='complete';},
