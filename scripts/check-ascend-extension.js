...
for(const file of ['background.js','content.js']){
  ...
  const sandbox={URL,crypto:require('node:crypto').webcrypto,location:{origin:'https://ascendtms.com'},chrome:{runtime:{id:'a'.repeat(32),
    getURL:file=>'chrome-extension://'+'a'.repeat(32)+'/'+file,
    connectNative(){connects++;throw Error('must not connect');},onConnect:{addListener(){}},onMessage:{addListener(fn){listener=fn;}}}}};
  vm.createContext(sandbox);
  sandbox.importScripts=(...names)=>names.forEach(name=>vm.runInContext(fs.readFileSync(root+'/'+name,'utf8'),sandbox));
  sandbox.importScripts('contract.js');sandbox.importScripts(file);
  let response;
