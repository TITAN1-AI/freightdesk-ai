...
function writeBanner(write) {
  ...
  return ({
    LOAD_OPENER_UNVERIFIED: 'Could not open the load workspace (no unique row, searchbox, or already-open Private Load Note / #scratch). Sit on 1763 Load Basics and retry. Harvest Active Loads is not required.',
    typed_but_not_saved: 'Private Load Note already has the text, but Save / Save & Exit was not clicked. Reload unpacked 0.1.7 and retry with whole-form approval.',
    ...
    BRIDGE_CLAIM_TIMEOUT: 'Bridge did not claim the write in time. Reload unpacked 0.1.7, keep the popup open, and retry.',
