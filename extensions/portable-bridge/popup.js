...
function writeBanner(write) {
  ...
  return ({
    LOAD_OPENER_UNVERIFIED: 'Could not open the load workspace. If Load Basics #scratch is on another tab, reload 0.1.8 and keep that tab open — Bridge must use the scratch tab, not the board searchbox.',
    typed_but_not_saved: 'Private Load Note already has the text, but Save / Save & Exit was not clicked. Reload unpacked 0.1.8 and retry with whole-form approval.',
    ...
    BRIDGE_CLAIM_TIMEOUT: 'Bridge did not claim the write in time. Reload unpacked 0.1.8, keep the popup open, and retry.',
